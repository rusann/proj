...
def alpha_scaled_opt_gaps(a,b, b_mas, o, p, gaps, cover):   
    ...
    if cover[0]>cover_cut:
        if o[0][0] == 9 or o[0][1] == 9:
            emission = 1.0
        else:
            emission = b_mas[int(cover[0]*10), :, o[0][0], o[0][1]]
        alpha[:,0] = emission * p
    else:
        if o[0][0] == 9:
            emission = 1.0
    ...
    for t in range(1, len(o)):   
        if usfl.point_in_set(t, gaps)==True:
            ...
            if cover[t] > cover_cut:
                for i in range(0, N):
                    if o[t][0] == 9 or o[t][1] == 9:
                        emission = 1.0
                    else:
                        emission = b_mas[int(cover[t]*10), i, o[t][0], o[t][1]]
                    alpha[i, t] = np.dot(alpha[:, t-1], a[:, i]) * emission
            else:
                for i in range(0, N):
                    if o[t][0] == 9:
                        emission = 1.0
    ...
    for t in range(len(o)-2,-1,-1):        
        if usfl.point_in_set(t, gaps)==True:
            ...
            if cover[t+1] > cover_cut:
                for i in range(0, N):
                    for l in range(0, N):
                        if o[t+1][0] == 9 or o[t+1][1] == 9:
                            emission = 1.0
                        else:
                            emission = b_mas[int(cover[t+1]*10), l, o[t+1][0], o[t+1][1]]
                        beta[i, t] += a[i, l] * emission * beta[l, t+1]
            else:
                for i in range(0, N):
                    for l in range(0, N):
                        if o[t+1][0] == 9:
            ...
            for i in range(0, N):
                for j in range(0, N):
                    if o[t+1][0] == 9 or o[t+1][1] == 9:
                        ...
                        emission = b[j, o[t+1][0], o[t+1][1]]
                    denom += alpha[i, t] * a[i, j] * emission * beta[j, t+1]    
            for i in range(0, N):
                for j in range(0, N):
                    if o[t+1][0] == 9 or o[t+1][1] == 9:
                        emission = 1.0
                        ...
                        emission = b[j, o[t+1][0], o[t+1][1]]
                    ksi[i, j, t] = (alpha[i, t] * a[i, j] * emission * beta[j, t+1]) / denom
        else:
