...
def viterbi_modified(V, initial_distribution, a, b_our_mas, b_Skov, archaic_cover):
    ...
    if archaic_cover[0]>cover_cut:
        # If either observation in V[0] is missing (==9), set emission probability to 1
        if V[0][0] == 9 or V[0][1] == 9:
            emission = np.ones(initial_distribution.shape)
        else:
            emission = b_our_mas[int(archaic_cover[0]*10),:, V[0][0],V[0][1]]
        omega[0, :] = np.log(initial_distribution * emission)
    else:
        if V[0][0] == 9:
            emission = np.ones(initial_distribution.shape)
        else:
            emission = b_Skov[:, V[0][0]]
        omega[0, :] = np.log(initial_distribution * emission)      
    ...
    for t in range(1, T):
        for j in range(M):
            if archaic_cover[t]>cover_cut:
                if V[t][0] == 9 or V[t][1] == 9:
                    emission = 1.0
                else:
                    emission = b_our_mas[int(archaic_cover[t]*10), j, V[t][0], V[t][1]]
                probability = omega[t - 1] + np.log(a[:, j]) + np.log(emission)
            else:
                if V[t][0] == 9:
                    emission = 1.0
                else:
                    emission = b_Skov[j, V[t][0]]
                probability = omega[t - 1] + np.log(a[:, j]) + np.log(emission)
